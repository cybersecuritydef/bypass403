...
def wrapper_expect_fuzz(url, hcode=(), hsize=(), cookie=None, redirect=False):
    ...
    for cmd in cmds:
        resp = requests.get(f"{url}expect://{cmd}", headers=common.USER_AGENT, cookies=cookie, allow_redirects=redirect)
        print(common.good_code(resp.status_code, f"expect://{cmd} [code:{resp.status_code} size:{len(resp.content)}]"))
        resp = requests.get(f"{url}expect://{cmd}%00", headers=common.USER_AGENT, cookies=cookie, allow_redirects=redirect)
...
def wrapper_glob_fuzz(url, hcode=(), hsize=(), cookie=None, redirect=False):
    ...
    resp = requests.get(f"{url}glob://*.*", headers=common.USER_AGENT, cookies=cookie, allow_redirects=redirect)
    print(common.good_code(resp.status_code, f"glob://*.* [code:{resp.status_code} size:{len(resp.content)}]"))
    resp = requests.get(f"{url}glob://*.*%00", headers=common.USER_AGENT, cookies=cookie, allow_redirects=redirect)
    print(common.good_code(resp.status_code, f"glob://*.*%00 [code:{resp.status_code} size:{len(resp.content)}]"))
    for d in dirs:
        resp = requests.get(f"{url}glob://{d}*", headers=common.USER_AGENT, cookies=cookie, allow_redirects=redirect)
        print(common.good_code(resp.status_code, f"glob://{d}* [code:{resp.status_code} size:{len(resp.content)}]"))
        resp = requests.get(f"{url}glob://{d}*%00", headers=common.USER_AGENT, cookies=cookie, allow_redirects=redirect)
...
def wrapper_http_fuzz(url, hcode=(), hsize=(), cookie=None, redirect=False):
    schemes = ["http", "https"]
    resp = requests.get(f"{url}{url}/server-status", headers=common.USER_AGENT, cookies=cookie, allow_redirects=redirect)
    ...
    resp = requests.get(f"{url}{url}/server-status%00", headers=common.USER_AGENT, cookies=cookie, allow_redirects=redirect)
    print(common.good_code(resp.status_code, f"{url}{url}/server-status%00 [code:{resp.status_code} size:{len(resp.content)}]"))
    resp = requests.get(f"{url}{url}/%00", headers=common.USER_AGENT, cookies=cookie, allow_redirects=redirect)
    ...
    for scheme in schemes:        
        for addr in common.ADDR_LOCALHOST:
            resp = requests.get(f"{url}{scheme}://{addr}/", headers=common.USER_AGENT, cookies=cookie, allow_redirects=redirect)
            print(common.good_code(resp.status_code, f"{url}{scheme}://{addr}/ [code:{resp.status_code} size:{len(resp.content)}]"))
            resp = requests.get(f"{url}{scheme}://{addr}/%00", headers=common.USER_AGENT, cookies=cookie, allow_redirects=redirect)
            print(common.good_code(resp.status_code, f"{url}{scheme}://{addr}/%00 [code:{resp.status_code} size:{len(resp.content)}]"))
            resp = requests.get(f"{url}{scheme}://{addr}/server-status", headers=common.USER_AGENT, cookies=cookie, allow_redirects=redirect)
            print(common.good_code(resp.status_code, f"{url}{scheme}://{addr}/server-status [code:{resp.status_code} size:{len(resp.content)}]"))
            resp = requests.get(f"{url}{scheme}://{addr}/server-status%00", headers=common.USER_AGENT, cookies=cookie, allow_redirects=redirect)
